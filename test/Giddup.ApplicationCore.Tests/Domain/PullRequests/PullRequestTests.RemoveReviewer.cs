...
{
    [Fact]
    public async Task RemoveReviewer_NotCreated_ReturnsNotCreatedError()
    {
        // Arrange
    ...
    public async Task RemoveReviewer_InvalidStatus_ReturnsNotActiveError(PullRequestStatus status)
    {
        // Arrange
        var command = new RemoveReviewerCommand(Guid.NewGuid());
    ...
    [Fact]
    public async Task RemoveReviewer_NotExistingReviewer_ReturnsNoEvents()
    {
        // Arrange
        var command = new RemoveReviewerCommand(Guid.NewGuid());
    ...
    public async Task RemoveReviewer_ReturnsReviewerRemovedEventAndCompletedEvent(string? reviewerUserId, ReviewerType reviewerType, ReviewerFeedback reviewerFeedback, CheckForLinkedWorkItemsMode checkForLinkedWorkItemsMode, string? workItemId)
    {
        // Arrange
        var command = new RemoveReviewerCommand(Guid.NewGuid());
        ...
        _ = Assert.IsType<ReviewerRemovedEvent>(events.First());
    ...
    public async Task RemoveReviewer_ReturnsReviewerRemovedEvent(AutoCompleteMode autoCompleteMode, string? reviewerUserId, ReviewerType reviewerType, ReviewerFeedback reviewerFeedback, CheckForLinkedWorkItemsMode checkForLinkedWorkItemsMode)
    {
        // Arrange
        var command = new RemoveReviewerCommand(Guid.NewGuid());
        var reviewers = reviewerUserId != null ? GetReviewers((command.UserId, ReviewerType.Required, ReviewerFeedback.None), (Guid.Parse(reviewerUserId), reviewerType, reviewerFeedback)) : GetReviewers((command.UserId, ReviewerType.Required, ReviewerFeedback.None));
        ...
        _ = Assert.IsType<ReviewerRemovedEvent>(@event);
    }
}
